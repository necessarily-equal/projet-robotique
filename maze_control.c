...
void navigate_to_next_junction(void)
{
    ...
    while (true) {
        ...
        if (left_ir<WALL_THRESHOLD || right_ir<WALL_THRESHOLD){
            break;
        }
    }
    stop_move();
...
char junction_action(void)
{
    ...
    if (left_ir<WALL_THRESHOLD && dist_obstacle_is_close()
        && right_ir>WALL_THRESHOLD) //Left turn
        return ACTION_LEFT;
    else if (left_ir>WALL_THRESHOLD && dist_obstacle_is_close() &&
